...
export const DndContext = memo(function DndContext({
  ...
  const active = useMemo(() => {
    if (activeId != null) {
      const data = node ? node.data : defaultData;
      return {
        id: activeId,
        data,
        rect: activeRects,
  ...
  const overNode = droppableContainers.getNodeFor(sensorContext.current.over && sensorContext.current.over.id);
  ...
  const overRect = over ? over.rect : null;
  const transform = adjustScale(appliedTranslate, overRect, activeNodeRect);
      ...
      const sensorInstance = new Sensor({
        ...
        onStart(initialCoordinates) {
          ...
          unstable_batchedUpdates(() => {
            ...
            dispatchMonitorEvent && dispatchMonitorEvent({ type: 'onDragStart', event });
          });
  ...
  return (
    <DndMonitorContext.Provider value={registerMonitorListener}>
      <InternalContext.Provider value={internalContext}>
        ...
        <RestoreFocus disabled={accessibility && accessibility.restoreFocus === false ? false : true} />
  ...
  function getAutoScrollerOptions() {
    const activeSensorDisablesAutoscroll =
      activeSensor && activeSensor.autoScrollEnabled === false;
